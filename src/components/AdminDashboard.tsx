...
import { MenuItem, CartItem, Reservation, Order, User, RestaurantInfo } from '../types';
import { 
  TrendingUp, 
  Users, 
  ShoppingBag, 
...
export default function AdminDashboard({
  ...
  const [activeSubTab, setActiveSubTab] = useState<'orders' | 'reservations' | 'users' | 'supervisors' | 'menu' | 'categories' | 'settings' | 'export'>('orders');
  ...
  const handleSaveSupervisorSubmit = (e: React.FormEvent) => {
    ...
    setSupervisorFormData({ name: '', username: '', phone: '', password: '', notes: '' });
  };
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden text-right">
              {users.filter(u => u.role === 'supervisor').length === 0 ? (
                <div className="p-12 text-center space-y-3">
                  ...
                  <p className="text-xs text-slate-400">اضغط على زر "إضافة مشرف جديد" بالأعلى لإنشاء أول حساب مشرف بيوزر وباسورد.</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-right border-collapse">
                      ...
                      {users
                        ...
                        .map(sup => (
                          <tr key={sup.id} className="hover:bg-slate-50/80 transition-colors">
                            ...
                            <td className="p-4 text-slate-500">{sup.createdAt || 'جديد'}</td>
              ...
              <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" dir="rtl">
                <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-4 text-right">
                  <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                    <h3 className="font-extrabold text-slate-900 text-base flex items-center gap-2">
                      <ShieldCheck className="w-5 h-5 text-purple-600" />
                      <span>{editingSupervisor ? 'تعديل بيانات المشرف' : 'إضافة مشرف جديد'}</span>
                    </h3>
                    <button
                      onClick={() => {
                        setIsAddingSupervisor(false);
                  ...
                  <form onSubmit={handleSaveSupervisorSubmit} className="space-y-3.5 text-xs">
                    ...
                    <div className="flex items-center gap-2 pt-3 border-t border-slate-100">
                      ...
                      >
                        {editingSupervisor ? 'تحديث بيانات المشرف' : 'حفظ وإنشاء حساب المشرف'}
                      </button>
                    </div>
                  </form>
                </div>
